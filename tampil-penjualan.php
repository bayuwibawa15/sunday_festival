...
<body>
			...
			<div class="main-content">
				<section class="section">
					<div class="section-header">
						<h1>Grafik Penjualan <?php echo $filter; ?></h1>
					</div>
	...
	if($_POST['kode'] == "perbulan") {
		...
		$sql_penjualan = mysqli_query($conn, "SELECT *, SUM(tb_penjualanbantu.harga_penjualan*jumlah_penjualan) AS jumlah FROM tb_penjualanbantu,tb_produk, tb_penjualan WHERE tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tanggal_penjualan LIKE '$tahun-$bulan-$filter_hari' GROUP BY nama_produk ORDER BY date(tanggal_penjualan) DESC");
		$total_penjualan = mysqli_fetch_array($sql_penjualan);

		// $sql_penjualan = mysqli_query($conn, "SELECT SUM(harga_penjualan*jumlah_penjualan) AS total FROM tb_penjualan, tb_penjualanbantu WHERE tb_penjualanbantu.id_penjualan=tb_penjualan.id_penjualan AND tanggal_penjualan LIKE '$tahun-$bulan-$filter_hari'");
		// $total_penjualan = mysqli_fetch_array($sql_penjualan);


			$column1[] = "'".$x."'";
			$column2[] = $total_penjualan['jumlah'];
			// $column3[] = $total_penjualan['total'];
		...
		for($x=1;$x<=12;$x++){
			...
			$ket = 'Bulan';
		// $sql_komen = "SELECT COUNT(*) AS komen FROM tb_bantukomentar,tb_bantu WHERE tb_bantu.kode=tb_bantukomentar.kode AND tanggal_komentar LIKE '$_POST[tahun]%' AND id_dinas='$row2[id_dinas]' AND jenis='$row2[jenis]'";
		$sql_penjualan = mysqli_query($conn, "SELECT *, SUM(tb_penjualanbantu.harga_penjualan*jumlah_penjualan) AS jumlah FROM tb_penjualanbantu,tb_produk, tb_penjualan WHERE tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tanggal_penjualan LIKE '$tahun-$filter_bulan%' GROUP BY nama_produk ORDER BY date(tanggal_penjualan) DESC");
		$total_penjualan = mysqli_fetch_array($sql_penjualan);
