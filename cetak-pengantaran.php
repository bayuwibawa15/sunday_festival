...
$pdf= new TCPDF('L','mm',array(250,350));

// set document information
$pdf->SetCreator(PDF_CREATOR);
$pdf->SetAuthor('Jafar');
$pdf->SetTitle('Laporan Pengantaran');
$pdf->SetSubject('Pengantaran');
...
if($_POST['kode'] == "perbulan") {
		...
		$query = mysqli_query($conn, "SELECT * FROM tb_penjualanbantu,tb_produk, tb_penjualan, tb_pelanggan, tb_toko, tb_pengantaran WHERE tb_pengantaran.id_penjualan=tb_penjualan.id_penjualan AND tb_toko.id_toko=tb_penjualan.id_toko AND tb_pelanggan.id_pelanggan=tb_penjualan.id_pelanggan AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tanggal_penjualan LIKE '$tahun-$bulan%' ORDER BY date(tanggal_penjualan) DESC");
		}else{
		$query = mysqli_query($conn, "SELECT * FROM tb_penjualanbantu,tb_produk, tb_penjualan, tb_pelanggan, tb_toko, tb_pengantaran WHERE tb_pengantaran.id_penjualan=tb_penjualan.id_penjualan AND tb_toko.id_toko=tb_penjualan.id_toko AND tb_pelanggan.id_pelanggan=tb_penjualan.id_pelanggan AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tanggal_penjualan LIKE '$tahun-$bulan%' AND tb_penjualan.id_toko='$id_toko' ORDER BY date(tanggal_penjualan) DESC");
		}
}else{
		$tahun = $_POST['tahun'];
		if($_SESSION['level'] == "Admin") {
		$query = mysqli_query($conn, "SELECT * FROM tb_penjualanbantu,tb_produk, tb_penjualan, tb_pelanggan, tb_toko, tb_pengantaran WHERE tb_pengantaran.id_penjualan=tb_penjualan.id_penjualan AND tb_toko.id_toko=tb_penjualan.id_toko AND tb_pelanggan.id_pelanggan=tb_penjualan.id_pelanggan AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tanggal_penjualan LIKE '$tahun%' ORDER BY date(tanggal_penjualan) DESC");
		}else{
		$query = mysqli_query($conn, "SELECT * FROM tb_penjualanbantu,tb_produk, tb_penjualan, tb_pelanggan, tb_toko, tb_pengantaran WHERE tb_pengantaran.id_penjualan=tb_penjualan.id_penjualan AND tb_toko.id_toko=tb_penjualan.id_toko AND tb_pelanggan.id_pelanggan=tb_penjualan.id_pelanggan AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tanggal_penjualan LIKE '$tahun%' AND tb_penjualan.id_toko='$id_toko' ORDER BY date(tanggal_penjualan) DESC");
		}
}
...
$tbl2 = '<table border="0">
		...
		<table border="0" style="text-align:center;">
			<tr>
				<td colspan="2" style="text-align:center;"><font size="12em"><b>LAPORAN PENGANTARAN PRODUK</b></font></td>
			</tr>
		</table>
		<br><br>
		';
				...
				$tbl2 = $tbl2.'<table border="1px" cellpadding="2" cellspacing="0">
					<tr style="text-align:center;vertical-align:middle;">
									<th style="width:50px;border: 1px solid #000;">No</th>
													...
													<th style="width:200px;border: 1px solid #000;">Nama Toko</th>
													<th style="width:200px;border: 1px solid #000;">Nama Produk</th>
													<th style="width:150px;border: 1px solid #000;">Nama Pelanggan</th>
													<th style="width:150px;border: 1px solid #000;">Status</th>
													<th style="width:100px;border: 1px solid #000;">Harga Penjualan</th>
													<th style="width:70px;border: 1px solid #000;">Jumlah Stok</th>
													<th style="width:100px;border: 1px solid #000;">Total</th>
					</tr>';
					$no = 1;
					while ($tampil = mysqli_fetch_array($query)){
						...
						$tbl2 = $tbl2.'<tr style="text-align:center;">
									...
									<td align="left">'.$tampil['nama_produk'].'</td>
									<td align="left">'.$tampil['nama_pelanggan'].'</td>
									<td align="left">'.$tampil['status_pengantaran'].'</td>
									<td align="right">'.number_format($tampil['harga_penjualan'],0,",",".").'</td>
									<td align="right">'.number_format($tampil['jumlah_penjualan'],0,",",".").'</td>
									<td align="right">'.number_format($tampil['harga_penjualan']*$tampil['jumlah_penjualan'],0,",",".").'</td>
						</tr>';
					}
					$tbl2 = $tbl2.'<tr style="text-align:center;">
...
ob_end_clean();
$pdf->Output('LAPORAN PENGANTARAN PRODUK.pdf', 'I');
}
