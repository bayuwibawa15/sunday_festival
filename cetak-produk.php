...
$pdf->SetAuthor('Jafar');
$pdf->SetTitle('Laporan Produk Terlaris');
$pdf->SetSubject('Produk Terlaris');
...
if($_POST['kode'] == "perbulan") {
		...
		$query = mysqli_query($conn, "SELECT *,COUNT(*) AS total FROM tb_penjualanbantu, tb_toko, tb_penjualan,tb_produk WHERE tb_penjualanbantu.id_produk=tb_produk.id_produk AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_toko.id_toko=tb_penjualanbantu.id_toko AND tanggal_penjualan LIKE '$tahun-$bulan%' GROUP BY nama_toko ORDER BY nama_toko ASC");
		$nama_toko = $nama_toko;
			}else{
				$query = mysqli_query($conn, "SELECT *,COUNT(*) AS total FROM tb_penjualanbantu, tb_toko, tb_penjualan,tb_produk WHERE tb_penjualanbantu.id_produk=tb_produk.id_produk AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_toko.id_toko=tb_penjualanbantu.id_toko AND tanggal_penjualan LIKE '$tahun-$bulan%' AND nama_toko='$_POST[nama_toko]' GROUP BY nama_toko ORDER BY nama_toko ASC");
		...
		if($_SESSION['level'] == "Admin") {
			...
			$query = mysqli_query($conn, "SELECT *,COUNT(*) AS total FROM tb_penjualanbantu, tb_toko, tb_penjualan,tb_produk WHERE tb_penjualanbantu.id_produk=tb_produk.id_produk AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_toko.id_toko=tb_penjualanbantu.id_toko AND tanggal_penjualan LIKE '$tahun%' AND tb_penjualan.id_toko='$id_toko' GROUP BY nama_toko ORDER BY nama_toko ASC");
			}
}
...
$tbl2 = '<table border="0">
		...
		<table border="0" style="text-align:center;">
			<tr>
				<td colspan="2" style="text-align:center;"><font size="12em"><b>LAPORAN PRODUK TERLARIS</b></font></td>
			</tr>
		</table>
		<br><br>
		';
				...
				$tbl2 = $tbl2.'<table border="1px" cellpadding="2" cellspacing="0">
					<tr style="text-align:center;vertical-align:middle;">
									<th style="width:50px;border: 1px solid #000;">No</th>
													<th style="width:300px;border: 1px solid #000;">Nama Produk</th>
													<th style="width:150px;border: 1px solid #000;">Harga Penjualan</th>
													<th style="width:130px;border: 1px solid #000;">Total Terjual</th>
													<th style="width:190px;border: 1px solid #000;">Total Keuntungan</th>
					</tr>';
					$no = 1;
					$total = 0;
					while ($tampil = mysqli_fetch_array($query)){
						$total+=$tampil['harga_penjualan']*$tampil['total'];
						$tbl2 = $tbl2.'<tr style="text-align:center;">
									...
									<td align="right">'.number_format($tampil['harga_penjualan']*$tampil['total'],0,",",".").'</td>
						</tr>';
					}
					$tbl2 = $tbl2.'<tr style="text-align:center;">
...
ob_end_clean();
$pdf->Output('LAPORAN PRODUK TERLARIS.pdf', 'I');
}
