...
$pdf->SetAuthor('Jafar');
$pdf->SetTitle('Laporan Kategori Paling Laku');
$pdf->SetSubject('Kategori Paling Laku');
...
if($_POST['kode'] == "perbulan") {
		...
		$query = mysqli_query($conn, "SELECT *,COUNT(*) AS total FROM tb_penjualanbantu, tb_toko, tb_penjualan,tb_produk,tb_kategori,tb_merk WHERE tb_produk.id_kategori=tb_kategori.id_kategori AND tb_produk.id_merk=tb_merk.id_merk AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_toko.id_toko=tb_penjualanbantu.id_toko AND tanggal_penjualan LIKE '$tahun-$bulan%' GROUP BY nama_kategori, nama_produk ORDER BY nama_toko ASC");
		}else{
		...
		$tahun = $_POST['tahun'];
		if($_SESSION['level'] == "Admin") {
			$query = mysqli_query($conn, "SELECT *,COUNT(*) AS total FROM tb_penjualanbantu, tb_toko, tb_penjualan,tb_produk,tb_kategori,tb_merk WHERE tb_produk.id_kategori=tb_kategori.id_kategori AND tb_produk.id_merk=tb_merk.id_merk AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_toko.id_toko=tb_penjualanbantu.id_toko AND tanggal_penjualan LIKE '$tahun%' GROUP BY nama_kategori, nama_produk ORDER BY nama_toko ASC");
			}else{
			$query = mysqli_query($conn, "SELECT *,COUNT(*) AS total FROM tb_penjualanbantu, tb_toko, tb_penjualan,tb_produk,tb_kategori,tb_merk WHERE tb_produk.id_kategori=tb_kategori.id_kategori AND tb_produk.id_merk=tb_merk.id_merk AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_toko.id_toko=tb_penjualanbantu.id_toko AND tanggal_penjualan LIKE '$tahun%' AND tb_penjualan.id_toko='$id_toko' GROUP BY nama_kategori, nama_produk ORDER BY nama_toko ASC");
			}
}
...
$tbl2 = '<table border="0">
		...
		<table border="0" style="text-align:center;">
			<tr>
				<td colspan="2" style="text-align:center;"><font size="12em"><b>LAPORAN KATEGORI PALING LAKU</b></font></td>
			</tr>
		</table>
		<br><br>
		';
				...
				$tbl2 = $tbl2.'<table border="1px" cellpadding="2" cellspacing="0">
					<tr style="text-align:center;vertical-align:middle;">
									<th style="width:50px;border: 1px solid #000;">No</th>
													...
													<th style="width:250px;border: 1px solid #000;">Kategori</th>
													<th style="width:250px;border: 1px solid #000;">Nama Produk</th>
													<th style="width:130px;border: 1px solid #000;">Jumlah Laku</th>
					</tr>';
					$no = 1;
					while ($tampil = mysqli_fetch_array($query)){
						...
						$tbl2 = $tbl2.'<tr style="text-align:center;">
									...
									<td align="left">'.$tampil['nama_kategori'].'</td>
									<td align="left">'.$tampil['nama_produk'].'</td>
									<td align="center">'.number_format($tampil['total'],0,",",".").'</td>
						</tr>';
					}
					$tbl2 = $tbl2.'<tr style="text-align:center;">
...
ob_end_clean();
$pdf->Output('LAPORAN KATEGORI PALING LAKU.pdf', 'I');
}
