...
$pdf= new TCPDF('L','mm',array(250,300));
...
if($_POST['kode'] == "perbulan") {
		...
		if($_SESSION['level'] == "Admin") {
			if($_POST['nama_toko'] == "Semua Toko") {
				$query = mysqli_query($conn, "SELECT * FROM tb_penjualanbantu,tb_produk, tb_penjualan, tb_toko WHERE tb_toko.id_toko=tb_penjualan.id_toko AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tanggal_penjualan LIKE '$tahun-$bulan%' ORDER BY date(tanggal_penjualan) DESC");
				$nama_toko = $nama_toko;
			}else{
				$query = mysqli_query($conn, "SELECT * FROM tb_penjualanbantu,tb_produk, tb_penjualan, tb_toko WHERE tb_toko.id_toko=tb_penjualan.id_toko AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tanggal_penjualan LIKE '$tahun-$bulan%' AND nama_toko='$_POST[nama_toko]' ORDER BY date(tanggal_penjualan) DESC");
		...
		if($_SESSION['level'] == "Admin") {
			if($_POST['nama_toko'] == "Semua Toko") {
				$query = mysqli_query($conn, "SELECT * FROM tb_penjualanbantu,tb_produk, tb_penjualan, tb_toko WHERE tb_toko.id_toko=tb_penjualan.id_toko AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tanggal_penjualan LIKE '$tahun%' ORDER BY date(tanggal_penjualan) DESC");
				$nama_toko = $nama_toko;
			}else{
				$query = mysqli_query($conn, "SELECT * FROM tb_penjualanbantu,tb_produk, tb_penjualan, tb_toko WHERE tb_toko.id_toko=tb_penjualan.id_toko AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tanggal_penjualan LIKE '$tahun%' AND nama_toko='$_POST[nama_toko]' ORDER BY date(tanggal_penjualan) DESC");
				$nama_toko = 'Cabang '.$nama_toko;
			}
		}else{
		$query = mysqli_query($conn, "SELECT * FROM tb_penjualanbantu,tb_produk, tb_penjualan, tb_toko WHERE tb_toko.id_toko=tb_penjualan.id_toko AND tb_penjualan.id_penjualan=tb_penjualanbantu.id_penjualan AND tb_penjualanbantu.id_produk=tb_produk.id_produk AND tanggal_penjualan LIKE '$tahun%' AND tb_penjualan.id_toko='$id_toko' ORDER BY date(tanggal_penjualan) DESC");
		}
}
...
$tbl2 = '<table border="0">
		...
		<table border="0">
			...
			<tr>
				...
				<td style="text-align:left;"><font size="10em">: '.$filter.' ('.$nama_toko.')</font></td>
			</tr>
		</table>
		<br><br><br>
		<table border="0" style="text-align:center;">
			<tr>
				<td colspan="2" style="text-align:center;"><font size="12em"><b>LAPORAN PENJUALAN PRODUK</b></font></td>
			</tr>
		</table>
		<br><br>
		';
				...
				$tbl2 = $tbl2.'<table border="1px" cellpadding="2" cellspacing="0">
					<tr style="text-align:center;vertical-align:middle;">
									<th style="width:50px;border: 1px solid #000;">No</th>
													...
													<th style="width:200px;border: 1px solid #000;">Nama Pegawai</th>
													<th style="width:200px;border: 1px solid #000;">Nama Produk</th>
													<th style="width:150px;border: 1px solid #000;">Harga Penjualan</th>
													<th style="width:100px;border: 1px solid #000;">Jumlah Penjualan</th>
													<th style="width:130px;border: 1px solid #000;">Total</th>
					</tr>';
					$no = 1;
					while ($tampil = mysqli_fetch_array($query)){
						...
						$tbl2 = $tbl2.'<tr style="text-align:center;">
									...
									<td align="left">'.$tampil['nama_pegawai'].'</td>
