...
$pdf= new TCPDF('L','mm',array(250,330));
...
if($_POST['kode'] == "perbulan") {
		$bulan = $_POST['bulan'];
		$tahun = $_POST['tahun'];
		$query = mysqli_query($conn, "SELECT * FROM tb_pembelianbantu,tb_produk, tb_pembelian, tb_pemasok WHERE tb_pemasok.id_pemasok=tb_pembelian.id_pemasok AND tb_pembelian.id_pembelian=tb_pembelianbantu.id_pembelian AND tb_pembelianbantu.id_produk=tb_produk.id_produk AND tanggal_pembelian LIKE '$tahun-$bulan%' ORDER BY date(tanggal_pembelian) DESC");
}else{
		$tahun = $_POST['tahun'];
		$query = mysqli_query($conn, "SELECT * FROM tb_pembelianbantu,tb_produk, tb_pembelian, tb_pemasok WHERE tb_pemasok.id_pemasok=tb_pembelian.id_pemasok AND tb_pembelian.id_pembelian=tb_pembelianbantu.id_pembelian AND tb_pembelianbantu.id_produk=tb_produk.id_produk AND tanggal_pembelian LIKE '$tahun%' ORDER BY date(tanggal_pembelian) DESC");
}
...
$tbl2 = '<table border="0">
		...
		<table border="0" style="text-align:center;">
			<tr>
				<td colspan="2" style="text-align:center;"><font size="12em"><b>LAPORAN PEMBELIAN PRODUK</b></font></td>
			</tr>
		</table>
		<br><br>
		';
				...
				$tbl2 = $tbl2.'<table border="1px" cellpadding="2" cellspacing="0">
					<tr style="text-align:center;vertical-align:middle;">
									<th style="width:50px;border: 1px solid #000;">No</th>
													...
													<th style="width:250px;border: 1px solid #000;">Nama Pemasok</th>
													<th style="width:150px;border: 1px solid #000;">Harga Pembelian</th>
													<th style="width:100px;border: 1px solid #000;">Jumlah Stok</th>
													<th style="width:130px;border: 1px solid #000;">Total</th>
					</tr>';
					$no = 1;
					$total=0;
					while ($tampil = mysqli_fetch_array($query)){
						$total+=$tampil['harga_pembelian']*$tampil['jumlah_pembelian'];
						$tbl2 = $tbl2.'<tr style="text-align:center;">
									<td>'.$no++.'</td>
									<td>'.tglIndonesia(date('d F Y', strtotime($tampil['tanggal_pembelian']))).'</td>
									<td align="left">'.$tampil['nama_produk'].'</td>
									<td align="left">'.$tampil['nama_pemasok'].'</td>
									<td align="right">'.number_format($tampil['harga_pembelian'],0,",",".").'</td>
									<td align="right">'.number_format($tampil['jumlah_pembelian'],0,",",".").'</td>
									<td align="right">'.number_format($tampil['harga_pembelian']*$tampil['jumlah_pembelian'],0,",",".").'</td>
						</tr>';
					}
					$tbl2 = $tbl2.'<tr style="text-align:center;">
